// ============================================================
// app.js - Enrutamiento y lógica principal (con backend)
// ============================================================
...
async function updateDownloadButton() {
  ...
  if (!plugin.paid) {
    ...
    if (isLoggedIn && currentUser) {
      hasAccess = await userHasPlugin(currentUser, plugin.id);
    }
  }
  ...
  if (plugin.paid && isLoggedIn && currentUser && await userHasPlugin(currentUser, plugin.id)) {
    var license = await getUserLicense(currentUser, plugin.id);
    if (license) {
...
async function updateUI() {
  ...
  if (isLoggedIn && currentUser) {
    ...
    var displayName = data.success ? (data.user.displayName || currentUser) : currentUser;
    userStatusText.textContent = displayName;
    statusDot.className = 'status-dot';
    var isAdminUser = await isAdmin(currentUser);
...
async function handleLogin() {
  ...
  if (isRegisterMode) {
    ...
    var result = await registerUser(username, password, email);
    if (result.success) {
      ...
      loginError.textContent = result.error || '❌ Error al registrar';
      loginError.style.color = '#f87171';
    }
    return;
  }
...
async function handleRedeem() {
  ...
  var result = await redeemCode(currentUser, code);
  if (result.success) {
    var usosMsg = '';
    ...
    redeemError.textContent = result.error || '❌ Error al canjear';
    redeemSuccess.textContent = '';
  }
}
...
async function handleCreateTicket() {
  ...
  var result = await createTicket(currentUser, title, message);
  if (result.success) {
    ticketSuccess.textContent = '✅ Ticket creado correctamente. ID: ' + result.ticket.id;
    ticketError.textContent = '';
    ticketTitle.value = '';
    ticketMessage.value = '';
    setTimeout(function() {
...
async function openMyTicketsModal() {
  var tickets = await getTicketsByUser(currentUser);
  if (tickets.length === 0) {
...
async function toggleMyTicketDetail(ticketId) {
...
async function reopenMyTicket(ticketId) {
...
// MIS LICENCIAS
// ============================================================
async function openMyLicensesModal() {
  var data = await getUserData(currentUser);
  if (!data.success || !data.user.licenses || Object.keys(data.user.licenses).length === 0) {
    ...
    for (var pluginId in data.user.licenses) {
      var license = data.user.licenses[pluginId];
...
async function init() {
  ...
  console.log('👤 Usuarios: admin/1234, user/1234, Fxrz/05052424 (en el servidor)');
